...
describe("ttsGemini", () => {
  ...
  it("returns 401 on missing auth", async () => {
    ...
    await (ttsGemini as any)(req, res);

    expect(res.statusCode).toBe(401);
    expect(res.body).toEqual({ error: "Unauthorized" });
  });

  it("returns 403 on blocked role", async () => {
    state.verifyIdToken.mockResolvedValue({ uid: "u-b", email: "b@example.com" });
    setUser("u-b", "blocked");
  ...
  it("forwards request to Gemini API", async () => {
  ...
  it("handles API errors gracefully (network failure -> 502)", async () => {
    state.verifyIdToken.mockResolvedValue({ uid: "u1", email: "u1@example.com" });
    setUser("u1", "user", 10);
